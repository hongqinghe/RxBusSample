...
import com.example.rxbus.rxbus.event.DeadEvent;
import com.example.rxbus.rxbus.event.EventType;
import com.example.rxbus.rxbus.event.ProducerEvent;
import com.example.rxbus.rxbus.event.SubscriberEvent;
import com.example.rxbus.rxbus.finder.Finder;
 ...
 *            使用注解：通过它去找 对应注册的一个方法@Subscribe    必须标明注解，方法名可以随意取
 *
 *    *******注意：使用RxBus为了效率问题，并不能在抽象类中进行事件的监听（如果使用可利用匿名内部类的方式添加监听器）
 *           子类无法继承父类的注册，如果需要只能自己实现注册机制
 *
 * <P> Author: gongtong
...
public class Bus {
    ...
    public void unregister(Object object) {
        ...
        for (Map.Entry<EventType, ProducerEvent> entry : producerInListener.entrySet()) {
            ...
            if (value == null || !value.equals(producer)) {
                throw new IllegalArgumentException("当前的对象已经解除注册或者未注册");
            }
            //在当前的集合中删除Produce
        ...
        for (Map.Entry<EventType, Set<SubscriberEvent>> entry : subscriberInListener.entrySet()) {
            ...
            if (currentSubscribers == null || !currentSubscribers.containsAll(eventMethodInListener)) {
                throw new IllegalArgumentException("当前的观察者为空或者未注册观察者");
            }
    ...
    public boolean isRegister(Object object) {
        ...
        Map<EventType, Set<SubscriberEvent>> allSubscribers = finder.findAllSubscribers(object);
        for (EventType type : allSubscribers.keySet()){
            Set<SubscriberEvent> subscriberEventSet = subscribersByType.get(type);
            if (subscriberEventSet!=null){
                if (subscriberEventSet.containsAll(allSubscribers.get(type))) {
                    return true;
                }
            }
        }
